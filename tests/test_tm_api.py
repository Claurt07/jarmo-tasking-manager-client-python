...
def test_v1_project_search_all():
    ...
    pages = pd.concat(tm.v2.project_search(dict(textSearch='gimcovid')))
...
def test_v2_project_search_first_page():
    ...
    page1 = next(tm.v2.project_search(dict(textSearch='gimcovid')))
...
def test_v2_project_search_draft():
    ...
    page1 = next(tm.v2.project_search(dict(textSearch='gimcovid', projectStatuses='' )))
    assert isinstance(page1, pd.DataFrame)
    assert isinstance(page1.status, pd.Series)
    assert len(page1) <= 14
